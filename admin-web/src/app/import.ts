...
export const columns: Record<ImportKind, string[]> = {
 ...
 students: ['campus_code','admission_number','student_name','class_name'],
...
export function validateImport(kind: ImportKind, grid: string[][]): {rows: ImportRow[],errors:string[]} {
 ...
 const header=grid[0].map(v=>v.trim().toLowerCase()); const required=columns[kind];
 if(header.length!==required.length || new Set(header).size!==header.length || required.some(c=>!header.includes(c)))
 ...
 const limits:Record<string,number>={campus_code:30,campus_name:120,admission_number:50,student_name:120,class_name:50,parent_name:120,parent_email:254,phone:40};
 grid.slice(1).forEach((values,i)=>{
  ...
  header.forEach((key,j)=>{row[key]=(values[j]??'').trim(); if(key==='parent_email')row[key]=row[key].toLowerCase();
   if(!row[key]&&key!=='phone')errors.push(`Row ${i+2}: ${key} is required.`);
   if(row[key].length>limits[key])errors.push(`Row ${i+2}: ${key} is too long.`);
   if(/^[=+@]/.test(row[key]) && key!=='phone')errors.push(`Row ${i+2}: ${key} must be plain text, not a formula.`);
  });
