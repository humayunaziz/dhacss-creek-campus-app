...
export class App implements OnInit {
 ...
 hasAccess(){return this.isAdmin()||this.assignments().length>0;}
 ...
 filteredStudents(){const q=this.search.toLowerCase();return this.students().filter(s=>(!this.campus||s['campus_id']===this.campus)&&(!q||`${s['full_name']} ${s['admission_number']} ${s['class_name']}`.toLowerCase().includes(q)));}
 ...
 async assignTeacher(){await this.act(async()=>{await this.db.rpc('assign_teacher',{teacher_email:this.teacherEmail,target_campus:this.campus,target_class:this.className});this.teacherEmail='';this.message.set('Teacher assigned to this class.');});if(!this.error())await this.load();}
